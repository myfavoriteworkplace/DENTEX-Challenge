...
def get_parser():
    parser = argparse.ArgumentParser(
        description="HierarchialDet inference for dental X-ray analysis"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of trained class levels (1=quadrant, 2=+enumeration, 3=+diagnosis)",
    )
    parser.add_argument(
        "--opts",
...
class Hierarchialdet:
    ...
    def validate_inputs(self):
        ...
        if not mha_files:
            errors.append(
                f"No .mha input files found in: {self.input_dir}"
            )
    ...
    def process(self):
        ...
        try:
            ...
            logger.info("Loaded image index with %d entries from %s", len(image_index), self.index_path)
        except FileNotFoundError as exc:
            logger.warning("Image index unavailable, slice indices will default to slice number. %s", exc)
            image_index = {}
